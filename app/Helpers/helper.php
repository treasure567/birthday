...
if (! function_exists('generate_message')) {
  function generate_message(Birthday $birthday, string $type = 'ai') {
    try {
      ...
      $treasure = ($birthday->whatsapp == '2348157002782') ? "He's the class rep for the department and he has been the most exceptional leader everyone loves. He's the one that even programmed this system to send birthday wishes for his coursemates. He'll be 20 years old, so assure him of a fulfiled adulthood" : '';
      $prompt = "Create a birthday message for {$birthday->name}, who is celebrating today, {$today}. {$birthday->name} is a {$birthday->gender}. $treasure. Mention that {$birthday->name} shares their birthday with famous persons celebrating today also mention the names of the famous persons also. Also, mention some notable computer or technological history that is remarkable to their birthday. Highlight how much they are valued by the Computer Science Department and express well wishes for many more years of success and celebration. Include a cheerful tone and wish them a fantastic day!. it's not a letter, so dont add sender name at the bottom";
